...
export default function SendMessage() {
  // Use HTMLAudioElement type for audioRef

  const handlePlayAudio = () => {
    const tone = new Audio('/message3.mp3');
    tone.playbackRate = 2
    tone.play();
  ...
  return (
    <div className="h-[10dvh] box-border flex gap-2 sm:gap-8 items-center p-4 sm:px-8 border-t">
      <textarea
        ...
        rows={1}
        cols={1}
        className="border rounded-full px-4 py-1 text-lg w-full resize-none"
        name=""
        id=""
      />
      <button className="group" onClick={handlePlayAudio}>
        <svg
          ...
          className="h-7 w-7 md:w-8 md:h-8"
